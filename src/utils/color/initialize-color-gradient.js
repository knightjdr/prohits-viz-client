...
// initializeBiDirection creates a two color hex gradient. The starting color is set using the
...
const initializeBiDirection = (colorSpace, numColors) => {
  let hEnd;
  let hStart;
  let s;
  ...
  for (let i = 0; i < halfColors; i += 1) {
    ...
    gradient[i] = convertHSLtoHex({ h: hStart, s, l: lightness });
  }
  gradient[halfColors] = convertHSLtoHex({ h: 0, s, l: 1 });
  startL = 1;
  const startIndex = halfColors + 1;
  for (let i = 0; i < halfColors; i += 1) {
    const lightness = roundNearest(startL - ((i + 1) * increment), 0.0001);
    gradient[i + startIndex] = convertHSLtoHex({ h: hEnd, s, l: lightness });
  }
  return gradient;
};

// initializeMonoDirection creates a single direction hex gradient. The color scale is set using the hue
...
const initializeMonoDirection = (colorSpace, numColors) => {
  let h;
  let s;
  if (colorSpace === 'green') {
...
/* initializeColorGradient defines a color gradient to use for fill values. It defines
** the colors to use via HSL and then converts those to HEX. */

const initializeColorGradient = (colorSpace, numColors, invert) => {
  let gradient;
  if (twoColor.includes(colorSpace)) {
    gradient = initializeBiDirection(colorSpace, numColors);
  } else {
    gradient = initializeMonoDirection(colorSpace, numColors);
  }
